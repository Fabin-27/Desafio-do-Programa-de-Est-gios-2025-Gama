...
public interface MetaRepository extends JpaRepository<Meta, Long>, JpaSpecificationExecutor<Meta> {
    ...
    @Query(value = "select meta from Meta meta left join fetch meta.metaAluno", countQuery = "select count(meta) from Meta meta")
    Page<Meta> findAllWithToOneRelationships(Pageable pageable);

    @Query("select meta from Meta meta left join fetch meta.metaAluno")
    List<Meta> findAllWithToOneRelationships();

    @Query("select meta from Meta meta left join fetch meta.metaAluno where meta.id =:id")
