...
type RestOf<T extends IAluno | NewAluno> = Omit<T, 'dtNascimento'> & {
...
export class AlunoService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/alunos', 'backend');
  ...
  protected convertDateFromClient<T extends IAluno | NewAluno | PartialUpdateAluno>(aluno: T): RestOf<T> {
    return {
      ...aluno,
      dtNascimento: aluno.dtNascimento?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAluno: RestAluno): IAluno {
    return {
      ...restAluno,
      dtNascimento: restAluno.dtNascimento ? dayjs(restAluno.dtNascimento) : undefined,
    };
  }
